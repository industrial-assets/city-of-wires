...
layout(set=0, binding=0) uniform PostProcessingUBO {
    ...
    float _pad_before_mats; // Padding to align mat4 to 16-byte boundary
    ...
    float _pad; // Padding for vec3 (std140 treats vec3 as vec4)
} ppUBO;
...
layout(set=0, binding=4) uniform sampler2D scatteringTexture;
layout(set=0, binding=5) uniform sampler2D transmittanceTexture;
layout(set=0, binding=6) uniform sampler2D anamorphicBloomTexture;
...
// Chromatic aberration: shift RGB channels radially from center
vec3 sampleWithChromaticAberration(sampler2D tex, vec2 uv, float strength) {
    vec2 center = vec2(0.5);
    vec2 offset = (uv - center) * strength;
    
    float r = texture(tex, uv + offset * 0.8).r;
    float g = texture(tex, uv).g;
    float b = texture(tex, uv - offset * 0.8).b;
    
    return vec3(r, g, b);
}

void main() {
    // Sample base inputs with chromatic aberration on scattering for lens distortion feel
    vec4 hdrColor = texture(hdrTexture, vUV);
    vec3 scattering = sampleWithChromaticAberration(scatteringTexture, vUV, ppUBO.chromaticAberrationStrength);
    float transmittance = texture(transmittanceTexture, vUV).r;
    vec3 anamorphicBloom = texture(anamorphicBloomTexture, vUV).rgb;
    
    // Debug modes based on multiplier value
    if (ppUBO.volumetricScatteringMultiplier > 40.0) {
        // Show raw scattering output (what raymarch accumulated)
        outColor = vec4(scattering * 10.0, 1.0);
        return;
    }
    
    if (ppUBO.volumetricScatteringMultiplier > 35.0) {
        // Show transmittance (how much light was absorbed)
        float t = 1.0 - transmittance;
        outColor = vec4(vec3(t * 100.0), 1.0);
        return;
    }
    
    transmittance = clamp(transmittance, 0.7, 1.0);
    
    // Legacy screen-space sun/shaft effects are disabled; the new volumetric pass handles this.
    vec3 lightShafts = vec3(0.0);
    vec3 sunDiskColor = vec3(0.0);
    
    // Combine scene with volumetrics and anamorphic bloom
    vec3 sceneColor = hdrColor.rgb * transmittance;
    vec3 finalColor = sceneColor + scattering * ppUBO.volumetricScatteringMultiplier + anamorphicBloom;
